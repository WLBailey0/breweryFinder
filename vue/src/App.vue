<template>
  <div id="app">
    ...
    <div id="nav">
      <router-link class='link' v-if="$store.state.token == ''" v-bind:to="{ name: 'home' }">Home</router-link>
      ...
      <router-link class='link' v-bind:to="{name: 'beers'}">Beers</router-link>
      <div class='dropdown' v-if="$store.state.token != ''">
        <button class="dropbtn">Account</button>
        <div class="dropdown-content">
          <router-link class='link' v-bind:to="{name: 'logout'}">Logout</router-link>
          <button id='deleteBtn' class='link' v-on:click='deleteUser'>Delete</button>
        </div>
      </div>
...
export default {
  ...
  methods : {
    deleteUser() {
      authService.delete(this.username).then(
        (response) => {
